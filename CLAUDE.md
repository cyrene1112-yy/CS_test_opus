...
## 9. Atomic Execution (原子执行)：

- 禁止大批量写入：单次代码修改（Write）不得超过 200 行或 3 个函数。
- 对于代码重构任务，若待重构代码文件已存在，永远用 Edit 做局部替换，不用 Write 全量覆。
- 若单次代码改动量较大，则对任务进行步骤拆分，每步一个独立逻辑单元，写完让用户确认再继续。
- 分步同步：每完成一个子任务（Sub-task），必须先执行文件写入并由用户确认，严禁累积多个步骤后一次性输出。
- 断点续传思维：如果任务被中断，下次启动应能通过已修改的文件和 tasks/todo.md 快速定位进度。
